...
import srangeldev.models.Atacar
import srangeldev.models.Defender
...
class SpellCaster(
    ...
    private fun castSpell() {
        ...
        if (enemy != null) {
            if (enemy is Defender) {
                //logger.debug {"Enemigo defensor"}
                val damage = calculateDamage()
                println(color.cyan("¡Hechizo impactado en $enemy con daño de $damage puntos!"))
                numberOfHits++

                if (enemy.shield > 0 && damage <= enemy.shield) {
                    //logger.debug {"Escudo mayor que 0 y daño menor o igual que el escudo"}
                    enemy.shield -= damage
                } else if (enemy.shield > 0 && damage > enemy.shield){
                    //logger.debug {"Escudo mayor que 0 y daño mayor que el escudo"}
                    var damageShield: Int = enemy.shield - damage
                    enemy.maxEnergy -= abs(damageShield)
                } else {
                    //logger.debug {"Enemigo sin escudo"}
    ...
    /**
     * funcion para colocar enemigos en el mapa
     */
    ...
    /**
     * funcion para crear el informe final
     */
    fun printReport() {
